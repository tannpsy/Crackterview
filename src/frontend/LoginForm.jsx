...
export default function LoginForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Else: regular user
      const userSideRes = await fetch("/user/api/auth/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    <div className="w-full max-w-md bg-white p-6 lg:p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <button
          ...
          className="w-full bg-crackterview-black text-white py-3 rounded-lg font-poppins text-sm font-bold hover:bg-gray-800 transition-colors duration-200 disabled:opacity-50"
        >
