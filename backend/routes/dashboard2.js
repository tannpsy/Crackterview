...
router.get('/candidates', isAuthenticated, isHR, async (req, res, next) => {
    try {
        const userId = req.user._id;
        const { keyword, page = 1, pageSize = 5, sortBy = 'appliedDate', sortOrder = 'desc' } = req.query;
        ...
        const candidates = await Candidate.aggregate([
            { $match: query },
            // NEW: Lookup the latest interview details
            {
                $lookup: {
                    from: 'interviews',
            ...
            },
            {
                $project: {
                    ...
                    position: 1,
                    appliedDate: 1,
                    // Use the candidate's hrRatings fields directly
                    hrRating: {
                        $avg: [
                            { $ifNull: ["$hrRatings.Fit for the Position", 0] },
                            { $ifNull: ["$hrRatings.Culture Fit", 0] },
                            { $ifNull: ["$hrRatings.Motivation", 0] },
                            { $ifNull: ["$hrRatings.Future Potential", 0] }
                        ]
                    },
                    // Use the latestInterview data for status
                    status: {
                        $cond: {
                            if: { $ne: ["$latestInterview.hrFeedbackProvided", false] },
                            then: "Reviewed",
                            else: "Unreviewed"
                        }
                    },
                    aiScore: { $ifNull: ["$latestInterview.overallAiScore", null] },
                    // Use the latestInterview data for email status
                    sendFeedbackStatus: {
                        $cond: {
                            if: { $ne: ["$latestInterview.emailSent", null] },
                            then: "Sent",
                            else: "Need Review"
                        }
                    },
...
// @route PUT /api/dashboard/candidates/:candidateId/feedback
// @desc Save HR feedback and optionally send an email
// @access Private (HR only)
router.put(
...
//         try {
//             const { interviewId } = req.params;
//             const { hrNotes, hrRatings } = req.body;
//             const userId = req.user._id;

//             // Find the Interview document to get the candidateId
//             const interview = await Interview.findOne({ candidateId: interviewId, userId: userId });

//             if (!interview) {
//                 return res.status(404).json({ message: 'Interview not found.' });
//             }
//             interview.hrNotes = hrNotes; // Assuming you have an hrFeedback field in your Candidate model

//             // Find the Candidate document using the candidateId from the interview
//             const candidate = await Candidate.findOne({ _id: interview.candidateId, createdBy: userId });

//             if (!candidate) {
//                 return res.status(404).json({ message: 'Candidate not found.' });
//             }

//             // Update the Candidate document with the new ratings and feedback
//             candidate.hrRatings = hrRatings;

//             await candidate.save();
            
//             // Also update the interview document to mark it as reviewed
//             interview.hrFeedbackProvided = true;
//             await interview.save();

//             res.status(200).json({
//                 message: "Feedback saved successfully on the candidate document.",
//                 candidate,
//             });
//         } catch (err) {
//             console.error('Error saving HR feedback:', err);
...
//             // Find the Interview document directly by its ID
//             const interview = await Interview.findOne({ candidateId: interviewId, userId: userId });
...
//             // Now find the Candidate document using the candidateId from the interview
//             const candidate = await Candidate.findOne({ _id: interview.candidateId, createdBy: userId });
...
//             // Update the interview document with the new feedback
//             interview.hrNotes = hrFeedback; // Save hrFeedback to the interview document
//             interview.hrFeedbackProvided = true; // Mark as reviewed
//             interview.emailSent = true; 
//             await interview.save();

//             // Extract the ratings from the interview document
//             const { 'Fit for the Position': fit, 'Culture Fit': culture, 'Motivation': motivation, 'Future Potential': potential } = interview.hrRatings;
...
//                 from: process.env.EMAIL_USER,
//                 to: candidate.email,
//                 subject: `Feedback for your application for the ${candidate.position} position`,
//                 html: `
//                     <h2>Dear ${candidate.name},</h2>
//                     <p>Thank you for your interest in the ${candidate.position} position. We have reviewed your interview and would like to provide some feedback.</p>
//                     <p><strong>HR Note:</strong></p>
//                     <p>${hrFeedback}</p>
//                     <p><strong>Detailed Ratings:</strong></p>
//                     <ul>
//                         <li><strong>Fit for the Position:</strong> ${fit} / 5</li>
//                         <li><strong>Culture Fit:</strong> ${culture} / 5</li>
//                         <li><strong>Motivation:</strong> ${motivation} / 5</li>
//                         <li><strong>Future Potential:</strong> ${potential} / 5</li>
//                     </ul>
//                     <p>Best regards,<br>The HR Team</p>
//                 `
//             };
